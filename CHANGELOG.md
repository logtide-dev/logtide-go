...
- `WrapTransport(base)`: an `http.RoundTripper` that injects the W3C `traceparent` header on outbound requests (active OTel span → context scope; no-op when neither is set or the header already exists)

## [0.9.3] - 2026-06-11

### Changed

- A `Retry-After` header on retryable responses now overrides the computed backoff delay
- HTTP `408` is now retried (alongside 429 and 5xx)

## [0.9.2] - 2026-06-11

### Added

- Every entry now carries `metadata.sdk = {"name": "logtide-go", "version": ...}` (caller-provided `sdk` key wins; the stamp is discarded on unmarshal and re-added on marshal)
